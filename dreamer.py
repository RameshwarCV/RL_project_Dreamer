...
class Dreamer:
    ...
    def world_model_loss(self, obs, acs, rews, nonterms):
        ...
        print("reward",rew_dist)
        print("obd_dist",obs_dist)
        if self.args.use_disc_model:
          disc_dist = self.discount_model(features)
        Dreamer_version=self.args.algo
        ...
        posterior_without_grad=self.rssm.detach_state(self.posterior)
        prior_without_grad=self.rssm.detach_state(prior)
        posterior_mean_without_grad=posterior_without_grad['mean']
        prior_mean_withoud_grad=prior_without_grad['mean']
        posterior_std_without_grad=posterior_without_grad['std']
        prior_std_without_grad= prior_without_grad['std']
        ...
        if(Dreamer_version=='DreamerV2'):
          kl_loss = self.args.kl_alpha *(torch.mean(distributions.kl.kl_divergence(
                               self.rssm.get_dist(posterior_mean_without_grad,posterior_std_without_grad), prior_dist)))
          kl_loss += (1-self.args.kl_alpha) * (torch.mean(distributions.kl.kl_divergence(
                               post_dist, self.rssm.get_dist(prior_mean_withoud_grad, prior_std_without_grad))))
...
#---------Additional Implementation ---------------------------------------
        ...
        if(Dreamer_version=='DreamerV2'):
          self.returns = td_lambda_new(imag_rews[:-1], imag_vals[:-1],discounts[:-1] \
                                         ,self.args.td_lambda, imag_vals[-1])
...
#----------Additional Implementation---------------------------------------
        ...
        actor_loss = -torch.mean(self.discounts * self.returns) ## want to maximize the discounted returns.
        return actor_loss
    ...
    def act_with_world_model(self, obs, prev_state, prev_action, explore=False):
        ...
        #---Additional Implementation
        if(self.args.algoself.args.algo=='DreamerV2'):
          features = torch.cat([posterior['stoch'], posterior['deter']], dim=-1)
        else:
          features =  posterior['deter']
        #---Additonal Implementation
...
def main():
    ...
    # Training parameters
    parser.add_argument('--total_steps', type=int, default=6000, help='total number of training steps')
