...
def td_lambda_new(rewards,values,discounts,td_lam,last_value):
    new_regularizer=(1-td_lam)*(1/(td_lam+0.000001))
    next_values = torch.cat([values[1:], last_value.unsqueeze(0)],0)  
    targets = rewards + discounts * next_values * new_regularizer
    rets =[]
    last_rew = last_value

    for t in range(rewards.shape[0]-1, -1, -1):
        last_rew = targets[t] + discounts[t] *  new_regularizer *(last_rew)
        rets.append(last_rew)
